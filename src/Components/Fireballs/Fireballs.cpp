...
// Fireballs
void Fireballs::add(FireballPtr fb) { mFireballs.push_back(std::move(fb)); }
void Fireballs::add(FireballPtr fb, const std::string& img) {
    fb->setImg(mAnims[img]);
    add(std::move(fb));
}
void Fireballs::add(FireballPtr fb, const AnimationData& img) {
    fb->setImg(mAnims[img]);
    add(std::move(fb));
}
...
void Fireballs::onResize(ResizeData data) {
...
void Fireballs::onUpdate(Time dt) {
...
void Fireballs::onRender(SDL_Renderer* renderer) {
    TextureBuilder tex;
    for (auto& fb : mFireballs) {
        tex.draw(fb->mImg);
    }
